...
class Prime_Cache_Admin_Settings {
	...
	private function tab_file_opt( $settings ) {
		...
		<form method="post" action="options.php">
			...
			<div class="pc-card">
				...
				<label class="pc-sw"><input type="checkbox" name="prime_cache_settings[async_css_free]" value="1" <?php checked( $settings['async_css_free'] ); ?>><span class="pc-sw__track"></span><span class="pc-sw__body"><b><?php esc_html_e( 'Load CSS Asynchronously (Free)', 'prime-cache' ); ?></b><small><?php esc_html_e( 'Convert non-first stylesheet link tags to non-render-blocking loading via the media-print and onload swap technique. The first stylesheet on the page is intentionally left synchronous to preserve LCP and prevent unstyled flash for above-the-fold content. Use the "Excluded CSS Files" list above for additional stylesheets that must remain render-blocking.', 'prime-cache' ); ?></small></span></label>
				<?php endif; ?>
			</div>
			...
			<div class="pc-card">
				...
				<?php
				$tweaks = array(
					array( 'local_jquery', __( 'Use Local jQuery', 'prime-cache' ), __( 'Restore WordPress bundled jQuery when the theme loads it from an external CDN. Eliminates external connection overhead (~600ms on mobile) by serving jQuery from the same origin.', 'prime-cache' ) ),
	...
	/**
	 * Suggest the optional drop-in serving mode on the plugin's own screen.
	 *
	 * Page caching works out of the box in standard mode (the plugin serves
	 * cached pages after WordPress core loads). Serving before WordPress loads
	 * (drop-in mode) additionally needs `define( 'WP_CACHE', true )` in
	 * wp-config.php — a change Prime Cache never makes itself. This notice
	 * shows the exact line for site owners who want the extra speed, and only
	 * on the Prime Cache settings screen so it never nags elsewhere.
	 */
	private function show_wp_cache_upgrade_notice() {
		...
		// Nothing to suggest when page caching is off, or when WP_CACHE is
		// already true — at runtime (drop-in mode active) or in wp-config.php
		// (the line was just added; it takes effect from the next request).
		$settings = prime_cache_get_settings();
