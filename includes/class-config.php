...
class Prime_Cache_Config {
	...
	private static function get_advanced_cache_content() {
		...
		return '<?php' . "\n"
			...
			. 'define( \'PRIME_CACHE_BLOG_ID\', ' . (int) $blog_id . ' );' . "\n"
	...
	public static function write_config_file( $settings ) {
		...
		// Host-independent config file. For multisite, blog_id is embedded in the
		// filename to prevent cross-site config collision.
		$blog_id = function_exists( 'get_current_blog_id' ) ? get_current_blog_id() : 1;
		$file    = $config_dir . 'site-config-' . $blog_id . '.php';
	...
	public static function delete_config_file() {
		...
		// Also clean up legacy config files (old site-config.php without blog_id).
		$legacy_file = $config_dir . 'site-config.php';
